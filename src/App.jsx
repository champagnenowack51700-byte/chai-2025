...
export default function App() {
  ...
  const exportVendangePDF = (annee, vAnneeComplete, destFilter="", clientFilter="") => {
    ...
    // Part reellement partie chez le negoce pour une ligne donnee (pas le marc total, notamment pour le "partiel")
    ...
    const colonnes = [
      ...
      {label:"Produits ajoutes", check:v=>destFilter!=="negoce"&&v.produitsAjoutes&&v.produitsAjoutes.length>0, html:v=>v.produitsAjoutes&&v.produitsAjoutes.length>0?v.produitsAjoutes.map(p=>p.nom+(p.dose?" "+p.dose:"")+(p.lot?" (Lot:"+p.lot+")":"")).join(", "):"-"},
